...
@pytest.mark.describe('Shopping List Calculator III - Outputs')
class TestPrint(TestCase):

    @pytest.mark.it('Print statements are correctly formatted')
    @patch('sys.stdout', new_callable=io.StringIO)
    def test_output(self, mock_stdout):
        from pset_basic_data_types.shopping_list.p3 import (
            item_name_1,
            item_name_2,
            item_name_3,
            item_name_4,
...
class TestInputsCalled(TestCase):
    ...
    @patch('pset_basic_data_types.shopping_list.p3.input')
